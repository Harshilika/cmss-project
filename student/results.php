...
if (isset($_POST['check_results'])) {
    // Retrieve and sanitize the roll ID and exam name
    $roll_id = isset($_POST['roll_id']) ? mysqli_real_escape_string($db_conn, $_POST['roll_id']) : '';
    $exam_name = isset($_POST['exam_name']) ? mysqli_real_escape_string($db_conn, $_POST['exam_name']) : '';

    // Query to get results for the entered roll ID and exam name
    $query = mysqli_query($db_conn, "
        SELECT p.title, m1.meta_value AS total_marks_obtained, m2.meta_value AS total_marks, m3.meta_value AS exam_name, m4.meta_value AS roll_id 
        FROM posts p 
        ...
        INNER JOIN metadata m4 ON p.id = m4.item_id AND m4.meta_key = 'roll_id' AND m4.meta_value = '$roll_id' 
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title">Enter Your Roll ID and Select Exam to Check Results</h3>
                    </div>
                    <div class="card-body">
                        <form action="" method="POST">
                            <div class="form-group">
                                <label for="roll_id">Roll ID</label>
                                <input type="text" name="roll_id" placeholder="Enter your Roll ID" required class="form-control">
                            </div>
                            <div class="form-group">
                            ...
                            <div class="table-responsive bg-white">
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th>S.No.</th>
                                            <th>Roll ID</th> <!-- Added Roll ID column -->
                                            <th>Student Name</th>
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($student_results as $result): ?>
                                            <tr>
                                                <td><?= $count++ ?></td>
                                                <td><?= htmlspecialchars($result['roll_id']) ?></td> <!-- Display Roll ID -->
                ...
                <?php elseif (isset($_POST['check_results'])): ?>
                    <div class="alert alert-warning">No results found for the provided Roll ID and exam.</div>
                <?php endif; ?>
            </div>
        </div>
    </div><!--/. container-fluid -->
